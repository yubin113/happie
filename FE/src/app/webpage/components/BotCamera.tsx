...
export default function BotCamera({ botId }: { botId: number }) {
  const imageSrc = dummyCameraFeeds[botId];

  return (
    <div className="w-full h-36 bg-gray-200 rounded-lg flex items-center justify-center shadow-md overflow-hidden">
      {imageSrc ? (
        <img
          src={imageSrc}
          alt={`로봇 ${botId} 카메라 화면`}
