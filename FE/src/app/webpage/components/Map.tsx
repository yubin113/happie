        ...
        <div className="relative w-full max-w-[550px] aspect-square rounded-lg border border-gray-300 bg-gray-100 overflow-hidden">
          ...
          {positions.map((pos) => (
            ...
            >
              ...
              <AnimatePresence>
                {hoveredId === pos.id && (
                  <motion.div key="tooltip" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} transition={{ duration: 0.25 }} className="absolute -top-14 left-1/2 -translate-x-1/2 whitespace-nowrap bg-white border border-gray-300 px-3 py-2 rounded-lg text-sm shadow-md z-20 text-center">
                    <div>🤖 로봇 {pos.id}</div>
                    <div className="text-xs text-gray-500">{statuses[pos.id]?.todo ?? "불러오는 중..."}</div>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
