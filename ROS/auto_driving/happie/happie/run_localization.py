...
class Localization:
    ...
    # 로직 4. 저장된 맵을 불러옵니다.
    def load_map(self):
        # Directly using the provided full path
        full_path = r"C:\Users\SSAFY\Desktop\catkin_ws\src\sub3\map\map.txt"
        
        # Open the map file for reading
        with open(full_path, 'r') as f:
            line = f.readlines()
        
        # Get the first line of the map file and split by spaces
        line_data = line[0].split()

        # Initialize the map data array
        map_data = [0 for i in range(int(self.map_size[0] * self.map_size[1]))]

        # Convert the line data into integers and populate map_data
        for num, data in enumerate(line_data):
            map_data[num] = int(data)

        # Convert map data into a numpy array and reshape it
        np_map_data = np.array(map_data)
        self.map = np.reshape(np_map_data, (300, 300))

        # Normalize the map values and convert to uint8 type
        self.map = ((100 - self.map) * 255.0 / 100.0).astype(np.uint8)

        # Convert grayscale map to BGR color map for visualization
