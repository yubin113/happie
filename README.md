...
### 1. rqt 개요
`rqt`는 ROS 2에서 제공하는 GUI 기반의 플러그인 도구로, 시스템 모니터링 및 디버깅을 용이하게 합니다. 다양한 rqt 플러그인을 사용하여 ROS 2의 토픽, 서비스, 액션 등을 실시간으로 시각화하고 조작할 수 있습니다.

#### (1) 주요 기능
- `rqt_graph` : 노드와 토픽 간 연결 구조를 시각적으로 표시
- `rqt_plot` : 특정 ROS 2 토픽의 데이터 변화를 그래프로 표시
- `rqt_console` : ROS 2 로그 메시지를 실시간으로 확인하고 필터링
- `rqt_reconfigure` : 동적으로 ROS 2 파라미터를 조정 가능
