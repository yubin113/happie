...
def compute_relative_pose(pose_i, pose_j):
    ...
    # 로직 2 : Rot 행렬, trans 벡터 추출
    R_i = T_i[:2, :2]  # 상위 2x2 회전 행렬
    t_i = -R_i @ T_i[:2, 2]  # 역변환된 이동 벡터

    # 로직 3 : pose_i 기준 pose_j의 Rot 행렬, trans 벡터 계산
    R_ij = R_i.T @ T_j[:2, :2]  # R_i의 전치행렬을 이용해 상대 회전 행렬 계산
    t_ij = R_i.T @ (T_j[:2, 2] + t_i)  # 상대 이동 벡터 계산

    # 로직 4 : 위의 Rot 행렬, trans 벡터로 pose_ij를 계산
    pose_ij = np.array([0.0, 0.0, 0.0])
    pose_ij[:2] = t_ij
    pose_ij[2] = np.degrees(np.arctan2(R_ij[1, 0], R_ij[0, 0]))  # 회전 각도 계산 후 degree 변환
...
class Localization:
    ...
    def _prediction(self, diff_pose):
        ...
        delta_dist = np.hypot(diff_pose[0], diff_pose[1])
        delta_angle1 = np.arctan2(diff_pose[1], diff_pose[0])
        delta_angle2 = np.radians(diff_pose[2]) - delta_angle1
        
        odom_cov = np.diag([0.1, 0.1, np.radians(5)])  # 오도메트리의 불확실성
        
        delta_angle1 = utils.limit_angular_range(delta_angle1)
        
        dist_noise_coeff = 0.02
        angle1_noise_coeff = np.radians(1.0)
        angle2_noise_coeff = np.radians(1.0)
        
        score_sum = 0

        for i in range(self.num_particle):
            delta_dist_noisy = delta_dist + np.random.normal(0, dist_noise_coeff * delta_dist)
            delta_angle1_noisy = delta_angle1 + np.random.normal(0, angle1_noise_coeff)
            delta_angle2_noisy = delta_angle2 + np.random.normal(0, angle2_noise_coeff)
            
            x = delta_dist_noisy * np.cos(delta_angle1_noisy)
            y = delta_dist_noisy * np.sin(delta_angle1_noisy)
            h = delta_angle1_noisy + delta_angle2_noisy
    ...
    def _resampling(self):
        ...
        for i in range(n_particles):
            darted_score = np.random.uniform(0, score_baseline)
            darted_idx = np.searchsorted(particle_scores, darted_score) 
    ...
    def update(self, pose, laser):
        ...
        # 현재 위치와 이전 위치 간의 차이를 계산
        diff_pose = pose - self.odom_before

        # diff_dist와 diff_angle 계산
        diff_dist = np.hypot(diff_pose[0], diff_pose[1])
    ...
    def turtlebot_status_callback(self, msg):
        ...
        if self.is_init_pose ==True and self.is_imu==True:
            if self.is_status == False :
                ...
                # imu callback에서 처리한 odom_theta로 로봇의 헤딩을 얻어서
                # odom_theta, odom_x, odom_y 업데이트
                linear_x = msg.linear_velocity  # 선속도 (m/s)
                angular_z = msg.angular_velocity  # 각속도 (rad/s)

                # 선속도와 각속도를 사용하여 위치 업데이트
                self.odom_x += linear_x * self.period * np.cos(self.odom_theta)
                self.odom_y += linear_x * self.period * np.sin(self.odom_theta)
                self.odom_theta += angular_z * self.period

                # 헤딩 각도를 -π에서 π 사이로 유지
                self.odom_theta = utils.limit_angular_range(self.odom_theta)
