...
def createLineIterator(P1, P2, img):
    ...
    itbuffer = np.empty((max(dYa, dXa), 3), dtype=np.int32)
    ...
    if P1X == P2X:
        ...
        if steepSlope:
            itbuffer[:, 1] = np.arange(P1Y, P2Y, -1 if negY else 1)
            itbuffer[:, 0] = ((itbuffer[:, 1] - P1Y) / slope + P1X).astype(np.int32)
        else:
            itbuffer[:, 0] = np.arange(P1X, P2X, -1 if negX else 1)
            itbuffer[:, 1] = ((itbuffer[:, 0] - P1X) * slope + P1Y).astype(np.int32)
    
    colX = (itbuffer[:, 0] >= 0) & (itbuffer[:, 0] < imageW)
    colY = (itbuffer[:, 1] >= 0) & (itbuffer[:, 1] < imageH)
    itbuffer = itbuffer[colX & colY]
...
class Mapping:
    def __init__(self, params_map):
        ...
        self.map_size = (np.array(params_map["MAP_SIZE"]) / self.map_resolution).astype(int)
    ...
    def update(self, pose, laser):
        ...
        pose_x = (pose[0] - self.map_center[0]) / self.map_resolution
        pose_y = (pose[1] - self.map_center[1]) / self.map_resolution
        laser_global_x = (laser_global[0, :] - self.map_center[0]) / self.map_resolution
        laser_global_y = (laser_global[1, :] - self.map_center[1]) / self.map_resolution
        
        for i in range(laser_global.shape[1]):
            p1 = np.array([pose_x, pose_y]).astype(int)
            ...
            self.map[avail_y[:-1].astype(int), avail_x[:-1].astype(int)] = 0.7
            self.map[avail_y[-1].astype(int), avail_x[-1].astype(int)] = 0.0
            print("avail_y:", avail_y)
            print("avail_x:", avail_x)
    ...
    def show_pose_and_points(self, pose, laser_global):
        ...
        # 각 레이저 포인트에 대해 원을 그림
        for i in range(laser_global.shape[1]):
            l_x = int(laser_global_x[i])
            l_y = int(laser_global_y[i])
            cv2.circle(map_bgr, (l_x, l_y), 1, (0, 255, 0), -1)  # 레이저 포인트는 초록색 원으로 표시
        
        # 포즈를 빨간색 원으로 표시
        cv2.circle(map_bgr, (int(pose_x), int(pose_y)), 2, (0, 0, 255), -1)
...
class Mapper(Node):
    def __init__(self):
        ...
        self.map_size = int(params_map["MAP_SIZE"][0] / params_map["MAP_RESOLUTION"] *
                            params_map["MAP_SIZE"][1] / params_map["MAP_RESOLUTION"])
...
# def save_map(node, file_path):
#     # 로직 12 : 맵 저장
#     pkg_path = r"C:\Users\SSAFY\Desktop\catkin_ws\src\sub3\sub3"  # 수정된 경로
#     back_folder = '..'
#     folder_name = 'map'
#     file_name = file_path
#     full_path = os.path.join(pkg_path, back_folder, folder_name, file_name)
